...
➤ Flawed Two-Factor Authentication Logic
This vulnerability occurs when the application does not properly bind the first authentication step (password login) to the second step (OTP verification). As a result, an attacker can bypass or manipulate the 2FA flow because the system does not reliably ensure that the same user who initiated the login is the one completing the second verification step.
